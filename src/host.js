...
function readBindFlag(filePath) {
  ...
  try {
    ...
    return { ok: true, bound: data.bound === true }
  } catch (err) {
    return { ok: false, bound: false }
  }
}
...
export default {
  ...
  apply(ctx) {
    ...
    // ---------- 路由注册（openai-codex，displayName=ChatGPT，transport=sse） ----------
    async function ensureCodexRoute() {
      ...
      try {
        ...
        if (existing && typeof existing.apiKeyEnv === 'string' && existing.apiKeyEnv.length > 0) {
          // 自我升级：桥接自有的旧默认配置 → 补齐 displayName=ChatGPT + transport=sse，保留其余字段；
          // 用户自定义配置（apiKeyEnv 非 OPENAI_CODEX_API_KEY）绝不覆盖；升级只发生一次
          if (existing.apiKeyEnv === 'OPENAI_CODEX_API_KEY') {
            const patch = {};
            if (existing.displayName === 'Codex') patch.displayName = 'ChatGPT';
            if (existing.transport !== 'sse') patch.transport = 'sse'; // 绕开 pi-ai 默认 WebSocket 通道（实测不稳）
            if (Object.keys(patch).length > 0) {
              try {
                await settings.mutate('llm-pi-ai', [{ op: 'set', path: ['providers', 'openai-codex'], value: Object.assign({}, existing, patch) }]);
              } catch (upErr) {
                codexBridgeState.error = { kind: 'settings', message: 'openai-codex 路由升级失败，稍后重试' };
                console.warn('[dsh-chatgpt-subscription] Codex 路由升级失败（稍后自动重试）', String((upErr && upErr.message) || upErr));
              }
            }
          }
        ...
        console.warn('[dsh-chatgpt-subscription] Codex 路由注册失败（稍后自动重试）', String((err && err.message) || err));
      }
    }
    ...
    async function syncCodexTokenOnce() {
      if (!codexBridgeState.routeConfigured) await ensureCodexRoute();
      const nowMs = Date.now();
      const nowSec = Math.floor(nowMs / 1000);
      const flag = readBindFlag(CODEX_BIND_FILE);
      if (!flag.bound) {
        codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: { kind: 'unbound', message: '未绑定 ChatGPT 订阅，请在 DSH 设置「订阅」页授权绑定' }, routeConfigured: codexBridgeState.routeConfigured };
        return;
      }
      const read = readCodexAuthFile(CODEX_AUTH_FILE);
      if (!read.ok) {
        codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: { kind: 'no-login', message: '绑定已失效：未找到登录凭证，请重新授权' }, routeConfigured: codexBridgeState.routeConfigured };
        return;
      }
      ...
      if (!access) {
        codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: { kind: 'no-key', message: '绑定已失效：缺少 access_token，请重新授权' }, routeConfigured: codexBridgeState.routeConfigured };
        return;
      }
      ...
      if (token && token !== codexInjectedToken) {
        try {
          ...
          if (!error) error = { kind: 'credentials', message: '凭据注入失败' };
        }
      }
    ...
    async function runCodexOAuthFlow(pkce, state, serverHandle, authorizeUrl, waitCode) {
      ...
      try {
        ...
        try {
          ...
          oauthLastError = { kind: 'credentials', message: '绑定成功但凭据注入失败' };
        }
        const expiresAtSec = codexExpiresAt(decodeJwtExp(exchanged.access_token), Date.parse(nowIso));
        codexBridgeState = { ok: true, lastSyncAt: Date.now(), expiresAt: expiresAtSec != null ? expiresAtSec * 1000 : null, error: null, routeConfigured: codexBridgeState.routeConfigured };
      } catch (err) {
        oauthLastError = { kind: 'exception', message: '绑定异常：' + String((err && err.message) || err) };
      } finally {
    ...
    async function unbindCodexRpc() {
      ...
      try {
        ...
        try { await ctx.credentials.unset('OPENAI_CODEX_API_KEY'); } catch (err) { /* 未配置该键时 unset 无害 */ }
        codexInjectedToken = null;
        codexBridgeState = { ok: false, lastSyncAt: Date.now(), expiresAt: null, error: { kind: 'unbound', message: '已解绑 ChatGPT 订阅，请重新授权绑定' }, routeConfigured: codexBridgeState.routeConfigured };
    ...
    const MUTATING = { unbindCodex: true }; // 修改状态的方法需同源（防跨站触发清标记/unset）
    ...
    // 卸载：绑定标记与 auth.json 是用户数据，保留（标记决定是否注入；解绑由用户 RPC 触发）
    return function () {};
  },
};

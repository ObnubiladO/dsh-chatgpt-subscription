  ...
  return new Promise(function (resolve) {
    ...
    const server = createServer(function (req, res) {
      ...
      try {
        ...
        respondOAuthPage(res, 400, 'Invalid OAuth callback URL')
        return
      }
      if (pathname !== OAUTH_CALLBACK_PATH) {
        respondOAuthPage(res, 404, 'Callback path not found')
        return
      }
      if (params.get('state') !== expectedState) {
        respondOAuthPage(res, 400, 'OAuth state verification failed. Please try again.')
        return
      }
      const code = params.get('code')
      if (!code) {
        respondOAuthPage(res, 400, 'Authorization code missing')
        return
      }
      respondOAuthPage(res, 200, 'Authorization received. You can close this page.')
      onCode(code)
    })
...
function respondOAuthPage(res, status, message) {
  ...
  const html = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>ChatGPT authorization</title></head>' +
    ...
    '<p style="color:#555">You can close this page and return to DSH to continue.</p>' +
    '</body></html>'
  ...
  apply(ctx) {
    ...
    async function ensureCodexRoute(flag) {
      ...
      if (!settings || typeof settings.get !== 'function' || typeof settings.mutate !== 'function') {
        return { ok: false, owned: false, message: 'Settings service is not ready. Retrying next cycle.' };
      }
      try {
        ...
        if (existing && existing.apiKeyEnv !== 'OPENAI_CODEX_API_KEY') {
          return { ok: false, owned: false, message: 'A custom openai-codex route uses a different apiKeyEnv than OPENAI_CODEX_API_KEY. The plugin will not overwrite it.' };
        }
        if (existing) {
        ...
        return { ok: false, owned: false, message: 'Could not register the openai-codex route. Retrying later.' };
      }
    }
    ...
    async function configureDefaultModel() {
      ...
      if (!service || typeof service.currentSelection !== 'function' || typeof service.saveSelection !== 'function') return { ok: false, previous: null, message: 'The default model service is not ready. Cannot safely switch to ChatGPT.' };
    ...
    async function syncCodexToken() {
      ...
      syncInFlight = syncCodexTokenOnce().catch(function () {
        codexBridgeState = { ok: false, lastSyncAt: Date.now(), expiresAt: null, error: { kind: 'exception', message: 'Token sync failed' }, routeConfigured: codexBridgeState.routeConfigured };
    ...
    async function syncCodexTokenOnce() {
      ...
      if (!flag.bound) {
        ...
        try { await removeOwnedCodexRoute(flag); codexRouteOwned = false; } catch (err) {
          codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: { kind: 'route-cleanup', message: 'Could not remove the route after disconnecting' }, routeConfigured: true };
          return;
        }
        codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: cleared ? { kind: 'unbound', message: 'Not connected. Sign in on the ChatGPT Subscription page in DSH Settings.' } : { kind: 'credentials-cleanup', message: 'Not connected, but old credentials could not be removed' }, routeConfigured: false };
        return;
      }
      ...
      if (!read.ok) {
        ...
        const error = !routeRemoved ? { kind: 'route-cleanup', message: 'Connection is no longer valid and the route could not be removed' } : cleared ? { kind: 'no-login', message: 'Connection is no longer valid: sign-in credentials not found. Please reauthorize.' } : { kind: 'credentials-cleanup', message: 'Sign-in credentials are missing and old credentials could not be removed' };
      ...
      if (!access) {
        ...
        let routeRemoved = true;
        try { await removeOwnedCodexRoute(flag); codexRouteOwned = false; } catch (err) { routeRemoved = false; }
        const error = !routeRemoved ? { kind: 'route-cleanup', message: 'Connection is no longer valid and the route could not be removed' } : cleared ? { kind: 'no-key', message: 'Connection is no longer valid: access_token is missing. Please reauthorize.' } : { kind: 'credentials-cleanup', message: 'access_token is missing and old credentials could not be removed' };
        codexBridgeState = { ok: false, lastSyncAt: nowMs, expiresAt: null, error: error, routeConfigured: !routeRemoved };
        return;
      }
      ...
      if (codexNeedsRefresh(expiresAtSec, nowSec)) {
        if (!refresh) {
          error = { kind: 'auth', message: 'Token expires soon, but refresh_token is missing. Please reauthorize.' };
        } else {
          ...
          if (pair) {
            try {
              ...
              error = { kind: 'write', message: 'Token refreshed, but could not be saved to auth.json' };
            }
          } else {
            ...
            if (reAuth && reAccess && !isNaN(reRefreshMs) && (isNaN(lastRefreshMs) || reRefreshMs > lastRefreshMs)) {
              ...
              error = { kind: 'auth', message: 'Token refresh failed (refresh_token may be invalid). Please reauthorize.' };
            }
          }
        }
      } else {
      ...
      if (!token && error) {
        const cleared = await clearInjectedCodexCredential(flag);
        if (!cleared) error = { kind: 'credentials-cleanup', message: 'Token is unusable and old credentials could not be removed' };
      }
      if (token && token !== codexInjectedToken) {
        const canClaim = await canClaimCodexCredential(flag);
        if (!canClaim) {
          error = { kind: 'credentials-conflict', message: 'OPENAI_CODEX_API_KEY is already configured. The plugin will not overwrite it.' };
        } else try {
          ...
          const cleared = await clearInjectedCodexCredential(flag);
          error = cleared ? { kind: 'credentials', message: 'Could not set DSH credentials' } : { kind: 'credentials-cleanup', message: 'Could not set DSH credentials or remove old credentials' };
        }
      }
    ...
    async function runCodexOAuthFlow(pkce, state, serverHandle, authorizeUrl, waitCode) {
      ...
      try {
        ...
        if (!code) { oauthLastError = { kind: 'timeout', message: 'Authorization timed out after 5 minutes. Please try again.' }; return; }
        const exchanged = await exchangeAuthorizationCode(code, pkce.verifier);
        if (!exchanged.ok) {
          oauthLastError = { kind: 'exchange', message: 'Token exchange failed (' + (exchanged.status != null ? 'HTTP ' + exchanged.status : 'network error') + '). Please try again.' };
          return;
        }
        ...
        try {
          ...
          oauthLastError = { kind: 'write', message: 'Authorized, but could not save auth.json' };
          return;
        }
        ...
        try {
          ...
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
          oauthLastError = { kind: 'default-model', message: 'Authorized, but could not switch the default model. Check Settings and try again.' };
          return;
        }
        if (!defaultModel.ok) {
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
        ...
        if (!canClaim) {
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
          oauthLastError = { kind: 'credentials-conflict', message: 'OPENAI_CODEX_API_KEY is already configured. The plugin will not overwrite it.' };
          return;
        }
        try {
          ...
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
          oauthLastError = { kind: 'write', message: 'Authorized, but could not save the connection flag' };
          return;
        }
        try {
          ...
          await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
          oauthLastError = { kind: 'credentials', message: 'Connection failed: could not set DSH credentials' };
        ...
        await rollbackBindingSetup(route, defaultModel, previousAuth, newAccessToken);
        oauthLastError = { kind: 'exception', message: 'Connection failed. Please try again.' };
      } finally {
    ...
    async function startCodexOAuthRpc() {
      if (oauthInFlight) return { ok: false, oauthInFlight: true, error: { kind: 'in-flight', message: 'Authorization in progress. Please wait.' } };
      oauthInFlight = true;
      oauthLastError = null;
      ...
      if (!serverHandle) {
        oauthInFlight = false;
        return { ok: false, oauthInFlight: false, error: { kind: 'port-busy', message: 'Callback port ' + port + ' is in use. Close the program using it (such as an active codex sign-in) and try again.' } };
      }
    ...
    async function unbindCodexRpc() {
      if (oauthInFlight) return { ok: false, error: { kind: 'in-flight', message: 'Authorization in progress. Complete it or wait for it to time out.' } };
      try {
        const flag = readBindFlag(CODEX_BIND_FILE);
        ...
        codexBridgeState = { ok: false, lastSyncAt: Date.now(), expiresAt: null, error: { kind: 'unbound', message: 'ChatGPT subscription disconnected. Sign in again to reconnect.' }, routeConfigured: false };
        return { ok: true, bound: false };
      } catch (err) {
        return { ok: false, error: { kind: 'exception', message: 'Could not disconnect. Please try again.' } };
      }
    }
